...
echo "    docker run -it --name litert_shell --user $(id -u):$(id -g) -e HOME=/litert_build -e USER=$(id -un) -e http_proxy=\"${http_proxy}\" -e https_proxy=\"${https_proxy}\" -e no_proxy=\"${no_proxy}\" -e HTTP_PROXY=\"${HTTP_PROXY}\" -e HTTPS_PROXY=\"${HTTPS_PROXY}\" -e NO_PROXY=\"${NO_PROXY}\" -v ${REPO_ROOT}:/litert_build litert_build_env bash"
